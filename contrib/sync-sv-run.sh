...
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! nebulasync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    NEBULA_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $NEBULA_PID
    echo nebulasync: waiting for nebula to exit cleanly
    while [ -e /proc/$NEBULA_PID ]; do sleep 0.1; done
    echo nebulasync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo nebulasync: compressing blockchainstate...
    tar cf blockchain.tar.bz2 --use-compress-prog=pbzip2 -C $HOME blockchain
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! nebulasync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    FILE_NAME=blockchain-$VERSION-`date '+%Y%m%d-%H%M%S'`.tar.bz2
    echo nebulasync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.bz2 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
    	echo NOTIFYALERT! nebulasync was unable to upload $FILE_NAME to s3://$S3_BUCKET
    	exit 1
    fi
    echo nebulasync: replacing current version of blockchain-latest.tar.bz2 with $FILE_NAME
    ...
    if [[ ! $? -eq 0 ]]; then
    	echo NOTIFYALERT! nebulasync was unable to overwrite the current blockchainstate with $FILE_NAME
    	exit 1
    fi
    ...
    echo nebulasync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifynebulasync: nebulasync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.bz2 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/nebula`
    kill -9 $RUN_SV_PID
  fi
