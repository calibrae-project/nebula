...
{
   public:
      database_api_impl( const calibrae::app::api_context& ctx  );
      ~database_api_impl();
      ...
      calibrae::chain::database&                _db;
      std::shared_ptr< calibrae::follow::follow_api > _follow_api;
...
database_api::database_api( const calibrae::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const calibrae::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< calibrae::follow::follow_api >( ctx );
   }
...
{
   return calibrae::protocol::get_config();
}
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<calibrae::chain::account_member_index>();
...
order_book database_api_impl::get_order_book( uint32_t limit )const
{
   ...
   auto max_sell = price::max( SBD_SYMBOL, NECTAR_SYMBOL );
   auto max_buy = price::max( NECTAR_SYMBOL, SBD_SYMBOL );
...
//   if( buy_itr != end ) idump((*buy_itr));
   ...
   while(  buy_itr != end && buy_itr->sell_price.base.symbol == NECTAR_SYMBOL && result.asks.size() < limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
      ...
      cur.sbd     = ( asset( itr->for_sale, NECTAR_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
...
//   wdump((trx)(available_keys));
   auto result = trx.get_required_signatures( CALIBRAE_CHAIN_ID,
                                              available_keys,
...
{
   trx.verify_authority( CALIBRAE_CHAIN_ID,
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).active  ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).owner   ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).posting ); },
                         CALIBRAE_MAX_SIG_CHECK_DEPTH );
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   asset pot;
   if( my->_db.has_hardfork( CALIBRAE_HARDFORK_0_17__774 ) )
   ...
   if( total_r2 > 0 )
   {
      ...
      if( my->_db.has_hardfork( CALIBRAE_HARDFORK_0_17__774 ) )
      {
         ...
         vshares = d.net_rshares.value > 0 ? calibrae::chain::util::evaluate_reward_curve( d.net_rshares.value, rf.author_reward_curve, rf.content_constant ) : 0;
      }
      else
         vshares = d.net_rshares.value > 0 ? calibrae::chain::util::evaluate_reward_curve( d.net_rshares.value ) : 0;
   ...
   if( d.parent_author != CALIBRAE_ROOT_POST_PARENT )
...
vector<discussion> database_api::get_discussions_by_promoted( const discussion_query& query )const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type(CALIBRAE_MAX_SHARE_SUPPLY) )  );
