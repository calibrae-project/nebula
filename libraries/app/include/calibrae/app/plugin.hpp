...
if( options.count(name) ) { \
      ...
      std::transform(ops.begin(), ops.end(), std::inserter(container, container.end()), &calibrae::app::dejsonify<type>); \
}
/// @}
...
#define CALIBRAE_DEFINE_PLUGIN( plugin_name, plugin_class ) \
   ...
   std::shared_ptr< calibrae::app::abstract_plugin > create_ ## plugin_name ## _plugin( app::application* app )  \
   { return std::make_shared< plugin_class >( app ); } \
   } }

#define DEFINE_PLUGIN_EVALUATOR( PLUGIN, OPERATION, X )                     \
class X ## _evaluator : public calibrae::chain::evaluator_impl< X ## _evaluator, OPERATION > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( database& db, PLUGIN* plugin )                       \
         : calibrae::chain::evaluator_impl< X ## _evaluator, OPERATION >( db ), \
           _plugin( plugin )                                                \
      {}                                                                    \
                                                                            \
