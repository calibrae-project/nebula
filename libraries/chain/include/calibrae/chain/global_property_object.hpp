...
namespace steemit { namespace chain {
   ...
   class dynamic_global_property_object : public object< dynamic_global_property_object_type, dynamic_global_property_object>
   {
      public:
         ...
         asset       virtual_supply             = asset( 0, NECTAR_SYMBOL );
         asset       current_supply             = asset( 0, NECTAR_SYMBOL );
         asset       confidential_supply        = asset( 0, NECTAR_SYMBOL ); ///< total asset held in confidential balances
         ...
         asset       total_vesting_fund_steem   = asset( 0, NECTAR_SYMBOL );
         asset       total_vesting_shares       = asset( 0, VESTS_SYMBOL );
         asset       total_reward_fund_steem    = asset( 0, NECTAR_SYMBOL );
         fc::uint128 total_reward_shares2; ///< the running total of REWARD^2
         asset       pending_rewarded_vesting_shares = asset( 0, VESTS_SYMBOL );
         asset       pending_rewarded_vesting_steem = asset( 0, NECTAR_SYMBOL );

         price       get_vesting_share_price() const
         {
            if ( total_vesting_fund_steem.amount == 0 || total_vesting_shares.amount == 0 )
               return price ( asset( 1000, NECTAR_SYMBOL ), asset( 1000000, VESTS_SYMBOL ) );
         ...
         uint16_t sbd_print_rate = CALIBRAE_100_PERCENT;
