...
{
   FC_ASSERT( permlink.size() > CALIBRAE_MIN_PERMLINK_LENGTH && permlink.size() < CALIBRAE_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( CALIBRAE_HARDFORK_0_1 ) )
   {
      FC_ASSERT( o.url.size() <= CALIBRAE_MAX_WITNESS_URL_LENGTH, "URL is too long" );
   }
   else if( o.url.size() > CALIBRAE_MAX_WITNESS_URL_LENGTH )
   {
   ...
   if ( _db.has_hardfork( CALIBRAE_HARDFORK_0_14__410 ) )
   {
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_15__465 ) )
   {
   ...
   const auto& new_account = _db.create< account_object >( [&]( account_object& acc )
   {
      ...
      if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_11__169 ) )
...
{
   FC_ASSERT( _db.has_hardfork( CALIBRAE_HARDFORK_0_17__818 ), "Account creation with delegation is not enabled until hardfork 17" );
   ...
   if( o.delegation.amount > 0 || !_db.has_hardfork( CALIBRAE_HARDFORK_0_19__997 ) )
   {
      _db.create< vesting_delegation_object >( [&]( vesting_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + CALIBRAE_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
{
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_1 ) ) FC_ASSERT( o.account != CALIBRAE_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( CALIBRAE_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();
   ...
   {
#ifndef IS_TEST_NET
      if( _db.has_hardfork( CALIBRAE_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > CALIBRAE_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( CALIBRAE_HARDFORK_0_15__465 ) ) )
   {
      for( auto a: o.active->account_auths )
      {
         _db.get_account( a.first );
   ...
   if( o.posting && ( _db.has_hardfork( CALIBRAE_HARDFORK_0_15__465 ) ) )
   {
      for( auto a: o.posting->account_auths )
      {
         _db.get_account( a.first );
...
{
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_10 ) )
   {
   ...
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_6__80 ) && comment.parent_author != CALIBRAE_ROOT_POST_PARENT )
   {
...
{ try {
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_5__55 ) )
   ...
   if( o.parent_author != CALIBRAE_ROOT_POST_PARENT )
   {
      ...
      if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < CALIBRAE_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",CALIBRAE_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < CALIBRAE_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",CALIBRAE_MAX_COMMENT_DEPTH) );
   }

   if( ( _db.has_hardfork( CALIBRAE_HARDFORK_0_17__926 ) ) && o.json_metadata.size() )
   ...
   if ( itr == by_permlink_idx.end() )
   {
      if( o.parent_author != CALIBRAE_ROOT_POST_PARENT )
      {
         ...
         if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12__177 ) && !_db.has_hardfork( CALIBRAE_HARDFORK_0_17__869 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( *parent ) != fc::time_point_sec::maximum(), "Discussion is frozen." );
      }

      if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == CALIBRAE_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > CALIBRAE_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) > CALIBRAE_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( CALIBRAE_HARDFORK_0_6__113 ) )
      {
         if( o.parent_author == CALIBRAE_ROOT_POST_PARENT )
             FC_ASSERT( (now - auth.last_post) > CALIBRAE_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
         else
             FC_ASSERT( (now - auth.last_post) > CALIBRAE_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else
      ...
      uint16_t reward_weight = CALIBRAE_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12__176 ) && !_db.has_hardfork( CALIBRAE_HARDFORK_0_17__733 ) && o.parent_author == CALIBRAE_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), CALIBRAE_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( CALIBRAE_POST_AVERAGE_WINDOW - post_delta_time ) ) / CALIBRAE_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + CALIBRAE_100_PERCENT );
         reward_weight = uint16_t( std::min( ( CALIBRAE_POST_WEIGHT_CONSTANT * CALIBRAE_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( CALIBRAE_100_PERCENT ) ) );
      }
      ...
      const auto& new_comment = _db.create< comment_object >( [&]( comment_object& com )
      {
         ...
         if ( o.parent_author == CALIBRAE_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.has_hardfork( CALIBRAE_HARDFORK_0_12__177 ) ?
               _db.head_block_time() + CALIBRAE_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
         ...
         if( _db.has_hardfork( CALIBRAE_HARDFORK_0_17__769 ) )
         {
            com.cashout_time = com.created + CALIBRAE_CASHOUT_WINDOW_SECONDS;
         }
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
   ...
   else // start edit case
   {
      ...
      if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_17__772 ) )
      {
         if( _db.has_hardfork( CALIBRAE_HARDFORK_0_14__306 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
         else if( _db.has_hardfork( CALIBRAE_HARDFORK_0_10 ) )
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   if( !account.mined && _db.has_hardfork( CALIBRAE_HARDFORK_0_1 ) )
   {
      const auto& props = _db.get_dynamic_global_properties();
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
      ...
      FC_ASSERT( account.vesting_shares > min_vests || ( _db.has_hardfork( CALIBRAE_HARDFORK_0_16__562 ) && o.vesting_shares.amount == 0 ),
   ...
   {
      if( _db.has_hardfork( CALIBRAE_HARDFORK_0_5__57 ) )
   ...
   {
      int vesting_withdraw_intervals = CALIBRAE_VESTING_WITHDRAW_INTERVALS_PRE_HF_16;
      if( _db.has_hardfork( CALIBRAE_HARDFORK_0_16__551 ) )
         vesting_withdraw_intervals = CALIBRAE_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
      ...
      _db.modify( account, [&]( account_object& a )
      {
         ...
         a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds(CALIBRAE_VESTING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < CALIBRAE_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= CALIBRAE_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( CALIBRAE_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= CALIBRAE_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( CALIBRAE_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < CALIBRAE_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( CALIBRAE_HARDFORK_0_3 ) ) {
...
{ try {
   ...
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
   {
#ifndef CLEAR_VOTES
   ...
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_11 ) )
      FC_ASSERT( elapsed_seconds >= CALIBRAE_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (CALIBRAE_100_PERCENT * elapsed_seconds) / CALIBRAE_VOTE_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_power + regenerated_power), int64_t(CALIBRAE_100_PERCENT) );
   FC_ASSERT( current_power > 0, "Account currently does not have voting power." );

   int64_t  abs_weight    = abs(o.weight);
   int64_t  used_power    = (current_power * abs_weight) / CALIBRAE_100_PERCENT;

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // used_power = (current_power * abs_weight / CALIBRAE_100_PERCENT) * (reserve / max_vote_denom)
   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * CALIBRAE_VOTE_REGENERATION_SECONDS / (60*60*24);
   FC_ASSERT( max_vote_denom > 0 );

   if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_14__259 ) )
   {
   ...
   int64_t abs_rshares    = ((uint128_t(voter.effective_vesting_shares().amount.value) * used_power) / (CALIBRAE_100_PERCENT)).to_uint64();
   if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;

   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_14__259 ) )
   {
      FC_ASSERT( abs_rshares > CALIBRAE_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or steem power." );
   }
   else if( _db.has_hardfork( CALIBRAE_HARDFORK_0_13__248 ) )
   {
      FC_ASSERT( abs_rshares > CALIBRAE_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or steem power." );
   }
   ...
   {
      if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12__177 ) )
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         if( _db.has_hardfork( CALIBRAE_HARDFORK_0_17__900 ) )
            FC_ASSERT( _db.head_block_time() < comment.cashout_time - CALIBRAE_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
         else if( _db.has_hardfork( CALIBRAE_HARDFORK_0_7 ) )
            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - CALIBRAE_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
      }
      ...
      if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12__177 ) && !_db.has_hardfork( CALIBRAE_HARDFORK_0_13__257)  )
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + CALIBRAE_CASHOUT_WINDOW_SECONDS_PRE_HF17;
         else
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + CALIBRAE_CASHOUT_WINDOW_SECONDS_PRE_HF12;
      ...
      _db.modify( comment, [&]( comment_object& c ){
         ...
         if( rshares > 0 )
            ...
            c.net_votes--;
         if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
      });
      ...
      _db.modify( root, [&]( comment_object& c )
      {
         ...
         if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_17__769 ) )
         {
            if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
            ...
            if( c.max_cashout_time == fc::time_point_sec::maximum() )
               c.max_cashout_time = _db.head_block_time() + fc::seconds( CALIBRAE_MAX_CASHOUT_WINDOW_SECONDS );
         }
      });
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible && _db.has_hardfork( CALIBRAE_HARDFORK_0_17__774 ) )
            curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;

         if( curation_reward_eligible )
         {
            if( comment.created < fc::time_point_sec(CALIBRAE_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
               ...
               if( _db.has_hardfork( CALIBRAE_HARDFORK_0_17__774 ) )
               {
                  const auto& reward_fund = _db.get_reward_fund( comment );
                  auto curve = !_db.has_hardfork( CALIBRAE_HARDFORK_0_19__1052 ) && comment.created > CALIBRAE_HF_19_SQRT_PRE_CALC
            ...
            if( _db.head_block_time() > fc::time_point_sec(CALIBRAE_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
               /// discount weight by time
               uint128_t w(max_vote_weight);
               uint64_t delta_t = std::min( uint64_t((cv.last_update - comment.created).to_seconds()), uint64_t(CALIBRAE_REVERSE_AUCTION_WINDOW_SECONDS) );

               w *= delta_t;
               w /= CALIBRAE_REVERSE_AUCTION_WINDOW_SECONDS;
   ...
   {
      FC_ASSERT( itr->num_changes < CALIBRAE_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );

      if( _db.has_hardfork( CALIBRAE_HARDFORK_0_6__112 ) )
      ...
      if( !_db.has_hardfork( CALIBRAE_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( CALIBRAE_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
{
   database& d = db();
   FC_ASSERT( d.has_hardfork( CALIBRAE_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( CALIBRAE_HARDFORK_0_5__59 ) )
   {
   ...
   if(itr == accounts_by_name.end())
   {
      db.create< account_object >( [&]( account_object& acc )
      {
         ...
         if( !db.has_hardfork( CALIBRAE_HARDFORK_0_11__169 ) )
            acc.recovery_account = "steem";
         else
   ...
   if( db.has_hardfork( CALIBRAE_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < CALIBRAE_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= CALIBRAE_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( CALIBRAE_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( CALIBRAE_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( CALIBRAE_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= CALIBRAE_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  auto calibrae_conversion_delay = CALIBRAE_CONVERSION_DELAY_PRE_HF_16;
  if( _db.has_hardfork( CALIBRAE_HARDFORK_0_16__551) )
     calibrae_conversion_delay = CALIBRAE_CONVERSION_DELAY;
...
{
   FC_ASSERT( !_db.has_hardfork( CALIBRAE_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void challenge_authority_evaluator::do_apply( const challenge_authority_operation& o )
{
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_14__307 ) ) FC_ASSERT( false, "Challenge authority operation is currently disabled." );
   ...
   if( o.require_owner )
   {
      ...
      FC_ASSERT( challenger.balance >= CALIBRAE_OWNER_CHALLENGE_FEE );
      FC_ASSERT( !challenged.owner_challenged );
      FC_ASSERT( _db.head_block_time() - challenged.last_owner_proved > CALIBRAE_OWNER_CHALLENGE_COOLDOWN );

      _db.adjust_balance( challenger, - CALIBRAE_OWNER_CHALLENGE_FEE );
      _db.create_vesting( _db.get_account( o.challenged ), CALIBRAE_OWNER_CHALLENGE_FEE );
  ...
  {
      FC_ASSERT( challenger.balance >= CALIBRAE_ACTIVE_CHALLENGE_FEE, "Account does not have sufficient funds to pay challenge fee." );
      FC_ASSERT( !( challenged.owner_challenged || challenged.active_challenged ), "Account is already challenged." );
      FC_ASSERT( _db.head_block_time() - challenged.last_active_proved > CALIBRAE_ACTIVE_CHALLENGE_COOLDOWN, "Account cannot be challenged because it was recently challenged." );

      _db.adjust_balance( challenger, - CALIBRAE_ACTIVE_CHALLENGE_FEE );
      _db.create_vesting( _db.get_account( o.challenged ), CALIBRAE_ACTIVE_CHALLENGE_FEE );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + CALIBRAE_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( CALIBRAE_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > CALIBRAE_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + CALIBRAE_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < CALIBRAE_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + CALIBRAE_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( CALIBRAE_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + CALIBRAE_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void delegate_vesting_shares_evaluator::do_apply( const delegate_vesting_shares_operation& op )
{
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      _db.create< vesting_delegation_expiration_object >( [&]( vesting_delegation_expiration_object& obj )
      {
         ...
         obj.expiration = std::max( _db.head_block_time() + CALIBRAE_CASHOUT_WINDOW_SECONDS, delegation->min_delegation_time );
      });
