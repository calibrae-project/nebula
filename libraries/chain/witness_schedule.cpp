...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( CALIBRAE_MAX_WITNESSES );
   ...
   for( auto itr = widx.begin();
        ...
        ++itr )
   {
      if( db.has_hardfork( CALIBRAE_HARDFORK_0_14__278 ) && (itr->signing_key == public_key_type()) )
         continue;
   ...
   while( mitr != pow_idx.end() && selected_miners.size() < wso.max_miner_witnesses )
   {
      ...
      if( selected_voted.find(mitr->id) == selected_voted.end() )
      {
         ...
         if( !( db.has_hardfork( CALIBRAE_HARDFORK_0_14__278 ) && db.get_witness( mitr->owner ).signing_key == public_key_type() ) )
         {
   ...
   for( auto witness_count = selected_voted.size() + selected_miners.size();
        sitr != schedule_idx.end() && witness_count < CALIBRAE_MAX_WITNESSES;
        ++sitr )
   {
      new_virtual_time = sitr->virtual_scheduled_time; /// everyone advances to at least this time
      processed_witnesses.push_back(sitr);

      if( db.has_hardfork( CALIBRAE_HARDFORK_0_14__278 ) && sitr->signing_key == public_key_type() )
   ...
   size_t expected_active_witnesses = std::min( size_t(CALIBRAE_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("CALIBRAE_MAX_WITNESSES",CALIBRAE_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );

   auto majority_version = wso.majority_version;

   if( db.has_hardfork( CALIBRAE_HARDFORK_0_5__54 ) )
   {
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < CALIBRAE_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % CALIBRAE_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
      if( db.has_hardfork(CALIBRAE_HARDFORK_0_4) )
      {
      ...
      if( props.num_pow_witnesses == 0 || db.head_block_num() > CALIBRAE_START_MINER_VOTING_BLOCK )
      {
         const auto& widx = db.get_index<witness_index>().indices().get<by_vote_name>();

         for( auto itr = widx.begin(); itr != widx.end() && (active_witnesses.size() < (CALIBRAE_MAX_WITNESSES-2)); ++itr )
         {
            if( itr->pow_worker )
               continue;
         ...
         if( sitr != schedule_idx.end() )
         {
            ...
            db.modify( *sitr, [&]( witness_object& wo )
            {
               ...
               if( db.has_hardfork( CALIBRAE_HARDFORK_0_2 ) )
      ...
      if( props.num_pow_witnesses > CALIBRAE_MAX_WITNESSES )
      {
         if( itr != pow_idx.end() )
         {
            db.modify( *itr, [&](witness_object& wit )
      ...
      while( itr != pow_idx.end() )
      {
         ...
         if( db.head_block_num() > CALIBRAE_START_MINER_VOTING_BLOCK || active_witnesses.size() >= CALIBRAE_MAX_WITNESSES )
            break;
         ++itr;
      }
      ...
      /*
         ...
         for( const string& w : active_witnesses )
            ...
            */
         // active witnesses has exactly CALIBRAE_MAX_WITNESSES elements, asserted above
         for( size_t i = 0; i < active_witnesses.size(); i++ )
         {
            _wso.current_shuffled_witnesses[i] = active_witnesses[i];
         }
