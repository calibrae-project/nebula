...
class debug_private_key_storage : public private_key_storage
{
   public:
      ...
      std::map< calibrae::chain::public_key_type, fc::ecc::private_key > key_table;
};

class debug_node_api_impl
{
   public:
      debug_node_api_impl( calibrae::app::application& _app );
      ...
      fc::optional< calibrae::chain::signed_block > debug_pop_block();
      //void debug_push_block( const calibrae::chain::signed_block& block );
      calibrae::chain::witness_schedule_object debug_get_witness_schedule();
      calibrae::chain::hardfork_property_object debug_get_hardfork_property_object();
      ...
      std::shared_ptr< calibrae::plugin::debug_node::debug_node_plugin > get_plugin();

      calibrae::app::application& app;
...
)
{
   ...
   if( (pubkey == calibrae::chain::public_key_type()) || (gen_pub == pubkey) )
   {
      result = gen_priv;
      return;
   }
...
debug_node_api_impl::debug_node_api_impl( calibrae::app::application& _app ) : app( _app )
{
#ifdef CALIBRAE_INIT_PRIVATE_KEY
   fc::ecc::private_key init_key = CALIBRAE_INIT_PRIVATE_KEY;
...
void debug_node_api_impl::debug_mine( debug_mine_result& result, const debug_mine_args& args )
{
   ...
   tx.sign( *priv, CALIBRAE_CHAIN_ID );
...
uint32_t debug_node_api_impl::debug_push_blocks( const std::string& src_filename, uint32_t count, bool skip_validate_invariants )
{
   ...
   std::shared_ptr< calibrae::chain::database > db = app.chain_database();
     ...
     )
   {
      ...
      calibrae::chain::block_log log;
      log.open( src_path );
      uint32_t first_block = db->head_block_num()+1;
      uint32_t skip_flags = calibrae::chain::database::skip_nothing;
      if( skip_validate_invariants )
         skip_flags = skip_flags | calibrae::chain::database::skip_validate_invariants;
      for( uint32_t i=0; i<count; i++ )
      {
         //fc::optional< calibrae::chain::signed_block > block = log.read_block( log.get_block_pos( first_block + i ) );
         uint64_t block_pos = log.get_block_pos( first_block + i );
         if( block_pos == calibrae::chain::block_log::npos )
         {
...
{
   return get_plugin()->debug_generate_blocks( debug_key, count, calibrae::chain::database::skip_nothing, 0, &key_storage );
}
...
{
   return get_plugin()->debug_generate_blocks_until( debug_key, head_block_time, generate_sparsely, calibrae::chain::database::skip_nothing, &key_storage );
}

fc::optional< calibrae::chain::signed_block > debug_node_api_impl::debug_pop_block()
{
   std::shared_ptr< calibrae::chain::database > db = app.chain_database();
   return db->fetch_block_by_number( db->head_block_num() );
}

/*void debug_node_api_impl::debug_push_block( const calibrae::chain::signed_block& block )
{
   app.chain_database()->push_block( block );
}*/

calibrae::chain::witness_schedule_object debug_node_api_impl::debug_get_witness_schedule()
{
   return app.chain_database()->get( calibrae::chain::witness_schedule_id_type() );
}

calibrae::chain::hardfork_property_object debug_node_api_impl::debug_get_hardfork_property_object()
{
   return app.chain_database()->get( calibrae::chain::hardfork_property_id_type() );
}
...
std::shared_ptr< calibrae::plugin::debug_node::debug_node_plugin > debug_node_api_impl::get_plugin()
{
...
void debug_node_api_impl::debug_set_hardfork( uint32_t hardfork_id )
{
   ...
   if( hardfork_id > CALIBRAE_NUM_HARDFORKS )
      return;
...
{
   return app.chain_database()->get( calibrae::chain::hardfork_property_id_type() ).last_hardfork >= hardfork_id;
}
...
debug_node_api::debug_node_api( const calibrae::app::api_context& ctx )
{
...
fc::optional< calibrae::chain::signed_block > debug_node_api::debug_pop_block()
{
   return my->debug_pop_block();
}

/*void debug_node_api::debug_push_block( calibrae::chain::signed_block& block )
{
   my->debug_push_block( block );
}*/

calibrae::chain::witness_schedule_object debug_node_api::debug_get_witness_schedule()
{
   return my->debug_get_witness_schedule();
}

calibrae::chain::hardfork_property_object debug_node_api::debug_get_hardfork_property_object()
{
