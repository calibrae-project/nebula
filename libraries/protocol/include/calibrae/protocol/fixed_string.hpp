...
namespace fc { namespace raw {
   ...
   inline void pack( Stream& s, const calibrae::protocol::fixed_string< Storage >& u )
   {
      pack( s, std::string( u ) );
   }

   template< typename Stream, typename Storage >
   inline void unpack( Stream& s, calibrae::protocol::fixed_string< Storage >& u )
   {
      std::string str;
      unpack( s, str );
      u = str;
   ...
   void to_variant(   const calibrae::protocol::fixed_string< Storage >& s, variant& v ) { v = std::string( s ); }

   template< typename Storage >
   void from_variant( const variant& v, calibrae::protocol::fixed_string< Storage >& s ) { s = v.as_string(); }
} // fc
