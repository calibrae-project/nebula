...
namespace steemit { namespace protocol {
   ...
   void account_create_operation::validate() const
   {
      ...
      FC_ASSERT( is_asset_type( fee, NECTAR_SYMBOL ), "Account creation fee must be STEEM" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, NECTAR_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= CALIBRAE_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= CALIBRAE_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= CALIBRAE_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_nebula_dollars <= CALIBRAE_100_PERCENT, "Percent cannot exceed 100%" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= CALIBRAE_100_PERCENT, "Weight is not a STEEMIT percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( memo.size() < CALIBRAE_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   {
      validate_account_name( from );
      FC_ASSERT( is_asset_type( amount, NECTAR_SYMBOL ), "Amount must be STEEM" );
      if ( to != account_name_type() ) validate_account_name( to );
      FC_ASSERT( amount > asset( 0, NECTAR_SYMBOL ), "Must transfer a nonzero amount" );
   }
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= CALIBRAE_100_PERCENT, "Percent must be valid steemit percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( fee >= asset( 0, NECTAR_SYMBOL ), "Fee cannot be negative" );
      props.validate();
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( CALIBRAE_EQUIHASH_N, CALIBRAE_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == CALIBRAE_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == CALIBRAE_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, NECTAR_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, NECTAR_SYMBOL ) ),
   ...
   {
      validate_account_name( owner );
      FC_ASSERT( ( is_asset_type( amount_to_sell, NECTAR_SYMBOL ) && is_asset_type( min_to_receive, SBD_SYMBOL ) )
         || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( min_to_receive, NECTAR_SYMBOL ) ),
   ...
   void limit_order_create2_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( amount_to_sell, NECTAR_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) ) ||
                 ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, NECTAR_SYMBOL ) ),
                 "Limit order must be for the STEEM:SBD market" );
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( nebula_amount.amount >= 0, "steem amount cannot be negative" );
      FC_ASSERT( sbd_amount.amount > 0 || nebula_amount.amount > 0, "escrow must transfer a non-zero amount" );
      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
      FC_ASSERT( (fee.symbol == NECTAR_SYMBOL) || (fee.symbol == SBD_SYMBOL), "fee must be STEEM or SBD" );
      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
      FC_ASSERT( nebula_amount.symbol == NECTAR_SYMBOL, "steem amount must contain STEEM" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( sbd_amount.amount >= 0, "sbd amount cannot be negative" );
      FC_ASSERT( nebula_amount.amount >= 0, "steem amount cannot be negative" );
      FC_ASSERT( sbd_amount.amount > 0 || nebula_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
      FC_ASSERT( nebula_amount.symbol == NECTAR_SYMBOL, "steem amount must contain STEEM" );
   }
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == NECTAR_SYMBOL || amount.symbol == SBD_SYMBOL );
      FC_ASSERT( memo.size() < CALIBRAE_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_steem, NECTAR_SYMBOL ), "Reward Steem must be STEEM" );
