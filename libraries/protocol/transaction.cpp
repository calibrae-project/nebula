...
calibrae::protocol::transaction_id_type calibrae::protocol::transaction::id() const
{
   auto h = digest();
...
const signature_type& calibrae::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)
{
...
signature_type calibrae::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         CALIBRAE_ASSERT( s.check_authority(id) ||
   ...
   {
      CALIBRAE_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      CALIBRAE_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         calibrae::protocol::verify_authority( operations, result, get_active, get_owner, get_posting, max_recursion );
...
{ try {
   calibrae::protocol::verify_authority( operations, get_signature_keys( chain_id ), get_active, get_owner, get_posting, max_recursion );
