...
#include <calibrae/wallet/reflect_util.hpp>
...
public:
   ...
   variant info() const
   {
      ...
      result["post_reward_fund"] = fc::variant(_remote_db->get_reward_fund( CALIBRAE_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }
   ...
   bool import_key(string wif_key)
   {
      ...
      calibrae::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         calibrae::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         calibrae::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         calibrae::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         calibrae::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < CALIBRAE_MAX_TIME_UNTIL_EXPIRATION );
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, CALIBRAE_CHAIN_ID );
      }
...
{ try {
   ...
   op.fee = my->_remote_db->get_chain_properties().account_creation_fee * asset( CALIBRAE_CREATE_ACCOUNT_WITH_NEBULA_MODIFIER, NECTAR_SYMBOL );
...
annotated_signed_transaction wallet_api::vote( string voter, string author, string permlink, int16_t weight, bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * CALIBRAE_1_PERCENT;
...
annotated_signed_transaction      wallet_api::send_private_message( string from, string to, string subject, string body, bool broadcast ) {
   ...
   op.id = CALIBRAE_PRIVATE_MESSAGE_COP_ID;
