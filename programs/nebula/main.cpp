...
int main(int argc, char** argv) {
   calibrae::plugin::initialize_plugin_factories();
   ...
   try {
      ...
      std::cerr << "------------------------------------------------------\n";
      auto initminer_private_key = graphene::utilities::key_to_wif( CALIBRAE_INIT_PRIVATE_KEY );
      std::cerr << "initminer public key: " << CALIBRAE_INIT_PUBLIC_KEY_STR << "\n";
      std::cerr << "initminer private key: " << initminer_private_key << "\n";
      std::cerr << "chain id: " << std::string(CALIBRAE_CHAIN_ID) << "\n";
      std::cerr << "blockchain version: " << fc::string( CALIBRAE_BLOCKCHAIN_VERSION ) << "\n";
      std::cerr << "------------------------------------------------------\n";
#else
      std::cerr << "------------------------------------------------------\n\n";
      ...
      app_options.add_options()
            ...
            ("version,v", "Print nebula version and exit.")
            ;

      bpo::variables_map options;

      for( const std::string& plugin_name : calibrae::plugin::get_available_plugins() )
         node->register_abstract_plugin( calibrae::plugin::create_plugin( plugin_name, node ) );
