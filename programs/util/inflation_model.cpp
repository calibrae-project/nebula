...
#define CALIBRAE_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/
   ...
   {
      uint32_t vesting_factor = (block_num < CALIBRAE_START_VESTING_BLOCK) ? 0 : 9;

      share_type curate_reward   = calc_percent_reward_per_block< CALIBRAE_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, CALIBRAE_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * vesting_factor;

      share_type content_reward  = calc_percent_reward_per_block< CALIBRAE_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, CALIBRAE_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * vesting_factor;

      share_type producer_reward = calc_percent_reward_per_block< CALIBRAE_PRODUCER_APR_PERCENT >( current_supply );
      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, CALIBRAE_MIN_PRODUCER_REWARD.amount );
      ...
      if( (block_num % CALIBRAE_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< CALIBRAE_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, CALIBRAE_MIN_POW_REWARD.amount );
         if( block_num < CALIBRAE_START_MINER_VOTING_BLOCK )
            pow_reward *= CALIBRAE_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % CALIBRAE_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< CALIBRAE_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, CALIBRAE_MIN_LIQUIDITY_REWARD.amount );
      }
   ...
   for( uint32_t b=1; b<10*CALIBRAE_BLOCKS_PER_YEAR; b++ )
   {
