...
{
   calibrae::protocol::transaction     tx;
   std::string                        wif;
};
...
struct tx_signing_result
{
   ...
   calibrae::protocol::public_key_type key;
   calibrae::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      sres.sig_digest = sreq.tx.sig_digest(CALIBRAE_CHAIN_ID);

      fc::ecc::private_key priv_key = *graphene::utilities::wif_to_key( sreq.wif );
      sres.sig = priv_key.sign_compact( sres.sig_digest );
      sres.key = calibrae::protocol::public_key_type( priv_key.get_public_key() );
