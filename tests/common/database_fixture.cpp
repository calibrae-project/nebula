...
//using namespace calibrae::chain::test;

uint32_t CALIBRAE_TESTING_GENESIS_TIMESTAMP = 1431700000;
...
clean_database_fixture::clean_database_fixture()
{
   ...
   auto wit_plugin = app.register_plugin< calibrae::witness::witness_plugin >();
   ...
   generate_block();
   db.set_hardfork( CALIBRAE_NUM_HARDFORKS );
   generate_block();
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db.head_block_time() - timestamp ).to_seconds() < CALIBRAE_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      if( db.has_hardfork( CALIBRAE_HARDFORK_0_17 ) )
      {
         account_create_with_delegation_operation op;
         op.new_account_name = name;
         op.creator = creator;
         op.fee = asset( fee, NECTAR_SYMBOL );
   ...
   )
{
   try
   {
      transfer( CALIBRAE_INIT_MINER_NAME, account_name, amount );
   ...
   )
{
   try
   {
      db_plugin->debug_update( [=]( database& db)
      {
         ...
         {
            if( amount.symbol == NECTAR_SYMBOL )
               a.balance += amount;
         ...
         if( amount.symbol == SBD_SYMBOL )
         {
            ...
            if( median_feed.current_median_history.is_null() )
               ...
               {
                  f.current_median_history = price( asset( 1, SBD_SYMBOL ), asset( 1, NECTAR_SYMBOL ) );
               });
         }
...
void database_fixture::vest( const string& from, const share_type& amount )
{
   try
   {
      ...
      op.from = from;
      op.to = "";
      op.amount = asset( amount, NECTAR_SYMBOL );
...
void database_fixture::set_price_feed( const price& new_price )
{
   try
   {
      for ( int i = 1; i < 8; i++ )
      {
         ...
         op.publisher = CALIBRAE_INIT_MINER_NAME + fc::to_string( i );
   ...
   generate_blocks( CALIBRAE_BLOCKS_PER_HOUR );
   BOOST_REQUIRE(
#ifdef IS_TEST_NET
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   {
      itr--;
      ops.push_back( fc::raw::unpack< calibrae::chain::operation >( db.get(itr->op).serialized_op ) );
   }
