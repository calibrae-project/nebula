...
#define PUSH_TX \
   calibrae::chain::test::_push_transaction

#define PUSH_BLOCK \
   calibrae::chain::test::_push_block
...
/*#define CALIBRAE_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CALIBRAE_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CALIBRAE_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define CALIBRAE_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CALIBRAE_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CALIBRAE_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   CALIBRAE_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   CALIBRAE_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
struct database_fixture {
   ...
   std::shared_ptr< calibrae::plugin::debug_node::debug_node_plugin > db_plugin;
