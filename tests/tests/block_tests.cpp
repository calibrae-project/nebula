...
{
   try {
      fc::time_point_sec now( CALIBRAE_TESTING_GENESIS_TIMESTAMP );
      ...
      signed_block cutoff_block;
      {
         ...
         // n.b. we generate CALIBRAE_MIN_UNDO_HISTORY+1 extra blocks which will be discarded on save
         for( uint32_t i = 1; ; ++i )
         {
...
BOOST_AUTO_TEST_CASE( fork_blocks )
{
   try {
      ...
      BOOST_CHECK_EQUAL(db2.head_block_num(), 13);
      {
         ...
         CALIBRAE_CHECK_THROW(PUSH_BLOCK( db1, b ), fc::exception);
      }
      BOOST_CHECK_EQUAL(db1.head_block_num(), 13);
      BOOST_CHECK_EQUAL(db1.head_block_id().str(), db1_tip);
...
BOOST_AUTO_TEST_CASE( switch_forks_undo_create )
{
   try {
      ...
      cop.creator = CALIBRAE_INIT_MINER_NAME;
      cop.owner = authority(1, init_account_pub_key, 1);
      cop.active = cop.owner;
      trx.operations.push_back(cop);
      trx.set_expiration( db1.head_block_time() + CALIBRAE_MAX_TIME_UNTIL_EXPIRATION );
      ...
      b = db2.generate_block(db2.get_slot_time(1), db2.get_scheduled_witness(1), init_account_priv_key, database::skip_nothing);
      db1.push_block(b);
      CALIBRAE_REQUIRE_THROW(db2.get(alice_id), std::exception);
      ...
      CALIBRAE_REQUIRE_THROW(db1.get(alice_id), std::exception);
...
BOOST_AUTO_TEST_CASE( duplicate_transactions )
{
   try {
      ...
      t.from = CALIBRAE_INIT_MINER_NAME;
      t.to = "alice";
      ...
      CALIBRAE_CHECK_THROW(PUSH_TX( db1, trx, skip_sigs ), fc::exception);
      ...
      CALIBRAE_CHECK_THROW(PUSH_TX( db1, trx, skip_sigs ), fc::exception);
      CALIBRAE_CHECK_THROW(PUSH_TX( db2, trx, skip_sigs ), fc::exception);
...
BOOST_FIXTURE_TEST_CASE( optional_tapos, clean_database_fixture )
{
   try
   {
      ...
      transfer( CALIBRAE_INIT_MINER_NAME, "alice", 1000000 );
      ...
      tx.set_expiration( db.head_block_time() + CALIBRAE_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( PUSH_TX( db, tx, database::skip_transaction_dupe_check ), fc::exception );
...
{ try {
   ...
   trx.operations.push_back(t);
   trx.set_expiration( db.head_block_time() + CALIBRAE_MAX_TIME_UNTIL_EXPIRATION );
   trx.validate();
   ...
   CALIBRAE_REQUIRE_THROW( db.push_transaction(trx, 0), tx_irrelevant_sig );
...
BOOST_FIXTURE_TEST_CASE( pop_block_twice, clean_database_fixture )
{
   try
   {
      ...
      db.get_account( CALIBRAE_INIT_MINER_NAME );
      // transfer from committee account to Sam account
      transfer( CALIBRAE_INIT_MINER_NAME, "sam", 100000 );
...
BOOST_FIXTURE_TEST_CASE( rsf_missed_blocks, clean_database_fixture )
{
   try
   {
      ...
      {
         return uint64_t( CALIBRAE_100_PERCENT ) * x / 128;
      };
      ...
      BOOST_CHECK_EQUAL( db.witness_participation_rate(), CALIBRAE_100_PERCENT );
...
BOOST_FIXTURE_TEST_CASE( skip_block, clean_database_fixture )
{
   try
   {
      ...
      int miss_blocks = fc::minutes( 1 ).to_seconds() / CALIBRAE_BLOCK_INTERVAL;
      ...
      BOOST_CHECK( db.head_block_time() == block_time + CALIBRAE_BLOCK_INTERVAL );
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_FIXTURE_TEST_CASE( hardfork_test, database_fixture )
{
   try
   {
      ...
      auto ahplugin = app.register_plugin< calibrae::account_history::account_history_plugin >();
      db_plugin = app.register_plugin< calibrae::plugin::debug_node::debug_node_plugin >();
      ...
      for( int i = CALIBRAE_NUM_INIT_MINERS; i < CALIBRAE_MAX_WITNESSES; i++ )
      {
         account_create( CALIBRAE_INIT_MINER_NAME + fc::to_string( i ), init_account_pub_key );
         fund( CALIBRAE_INIT_MINER_NAME + fc::to_string( i ), CALIBRAE_MIN_PRODUCER_REWARD.amount.value );
         witness_create( CALIBRAE_INIT_MINER_NAME + fc::to_string( i ), init_account_priv_key, "foo.bar", init_account_pub_key, CALIBRAE_MIN_PRODUCER_REWARD.amount );
      }
      ...
      BOOST_REQUIRE( !db.has_hardfork( CALIBRAE_HARDFORK_0_1 ) );

      BOOST_TEST_MESSAGE( "Generate blocks up to the hardfork time and check hardfork still not applied" );
      generate_blocks( fc::time_point_sec( CALIBRAE_HARDFORK_0_1_TIME - CALIBRAE_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db.has_hardfork( CALIBRAE_HARDFORK_0_1 ) );
      BOOST_REQUIRE( get_last_operations( 1 )[0].get< custom_operation >().data == vector< char >( op_msg.begin(), op_msg.end() ) );
      BOOST_REQUIRE( db.get(itr->op).timestamp == db.head_block_time() - CALIBRAE_BLOCK_INTERVAL );
   }
   FC_LOG_AND_RETHROW()
}
