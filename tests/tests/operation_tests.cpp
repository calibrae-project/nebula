...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db.get_account( CALIBRAE_INIT_MINER_NAME );
      ...
      op.creator = CALIBRAE_INIT_MINER_NAME;
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db.get_account( CALIBRAE_INIT_MINER_NAME ).balance.amount + 1, NECTAR_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      tx.sign( init_account_priv_key, db.get_chain_id() );
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      tx.sign( active_key, db.get_chain_id() );
      tx.sign( active_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      tx.sign( active_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / CALIBRAE_BLOCK_INTERVAL );
      ...
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.sign( bob_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
      tx.signatures.clear();
      tx.sign( bob_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db.head_block_time() + fc::seconds( CALIBRAE_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / CALIBRAE_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = calibrae::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / CALIBRAE_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      tx.set_expiration( db.head_block_time() + CALIBRAE_MIN_TRANSACTION_EXPIRATION_LIMIT );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::assert_exception );


      BOOST_TEST_MESSAGE( "--- Test success deleting a comment with negative rshares" );

      generate_block();
      vote.weight = -1 * CALIBRAE_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      ...
      generate_blocks( CALIBRAE_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / CALIBRAE_BLOCK_INTERVAL );
      ...
      generate_blocks( CALIBRAE_CASHOUT_WINDOW_SECONDS / CALIBRAE_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         op.author = "bob";
         op.permlink = "foo";
         op.weight = CALIBRAE_100_PERCENT;
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db.get_chain_id() );
         ...
         int64_t max_vote_denom = ( db.get_dynamic_global_properties().vote_power_reserve_rate * CALIBRAE_VOTE_REGENERATION_SECONDS ) / (60*60*24);
         ...
         BOOST_REQUIRE( alice_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / CALIBRAE_100_PERCENT );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / CALIBRAE_100_PERCENT );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db.head_block_time() + CALIBRAE_MIN_VOTE_INTERVAL_SEC );
         ...
         op.weight = CALIBRAE_100_PERCENT / 2;
         op.voter = "alice";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         BOOST_REQUIRE( db.get_account( "alice" ).voting_power == old_voting_power - ( ( old_voting_power + max_vote_denom - 1 ) * CALIBRAE_100_PERCENT / ( 2 * max_vote_denom * CALIBRAE_100_PERCENT ) ) );
         BOOST_REQUIRE( bob_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - db.get_account( "alice" ).voting_power ) / CALIBRAE_100_PERCENT );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db.head_block_time() + fc::seconds( ( CALIBRAE_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         uint128_t new_cashout_time = db.head_block_time().sec_since_epoch() + CALIBRAE_CASHOUT_WINDOW_SECONDS;

         BOOST_REQUIRE( new_bob.voting_power == CALIBRAE_100_PERCENT - ( ( CALIBRAE_100_PERCENT + max_vote_denom - 1 ) / max_vote_denom ) );
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + new_bob.vesting_shares.amount.value * ( old_voting_power - new_bob.voting_power ) / CALIBRAE_100_PERCENT );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * CALIBRAE_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         new_cashout_time = db.head_block_time().sec_since_epoch() + CALIBRAE_CASHOUT_WINDOW_SECONDS;
         auto sam_weight /*= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();*/
                         = ( ( uint128_t( new_sam.vesting_shares.amount.value ) * ( ( CALIBRAE_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) ) / CALIBRAE_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( new_sam.voting_power == CALIBRAE_100_PERCENT - ( ( CALIBRAE_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) );
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (CALIBRAE_100_PERCENT * ( db.head_block_time() - db.get_account( "alice").last_vote_time ).to_seconds() ) / CALIBRAE_VOTE_REGENERATION_SECONDS;
         ...
         auto new_rshares = ( ( fc::uint128_t( db.get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / CALIBRAE_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( db.get_comment( "alice", string( "foo" ) ).cashout_time == db.get_comment( "alice", string( "foo" ) ).created + CALIBRAE_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( CALIBRAE_1_PERCENT * 25 * ( new_alice.voting_power ) / CALIBRAE_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         auto alice_voting_power = new_alice.voting_power - used_power;

         op.voter = "alice";
         op.weight = CALIBRAE_1_PERCENT * 25;
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         tx.signatures.clear();
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.vesting_shares.amount.value ) * used_power ) / CALIBRAE_100_PERCENT ).to_uint64();
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( CALIBRAE_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / CALIBRAE_100_PERCENT;
         used_power = ( used_power + max_vote_denom - 1 ) / max_vote_denom;
         alice_voting_power -= used_power;

         op.weight = CALIBRAE_1_PERCENT * -75;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db.get_chain_id() );
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - CALIBRAE_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + CALIBRAE_BLOCK_INTERVAL ), true );
         ...
         op.weight = -1 * CALIBRAE_100_PERCENT;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db.get_chain_id() );
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db.head_block_time() + CALIBRAE_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( CALIBRAE_VESTING_WITHDRAW_INTERVALS * 2 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db.head_block_time() + CALIBRAE_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( CALIBRAE_VESTING_WITHDRAW_INTERVALS * 3 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db.head_block_time() + CALIBRAE_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = asset( CALIBRAE_MIN_ACCOUNT_CREATION_FEE + 10, NECTAR_SYMBOL);
      op.props.maximum_block_size = CALIBRAE_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == CALIBRAE_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == CALIBRAE_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == CALIBRAE_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = CALIBRAE_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      tx.sign( bob_private_key, db.get_chain_id() );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == CALIBRAE_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = CALIBRAE_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      tx.sign( bob_private_key, db.get_chain_id() );
      ...
      BOOST_REQUIRE( db.get_witness( CALIBRAE_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db.get_witness( CALIBRAE_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db.head_block_time() + CALIBRAE_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + CALIBRAE_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db.head_block_time() + CALIBRAE_OWNER_UPDATE_LIMIT + fc::seconds( CALIBRAE_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db.head_block_time() + CALIBRAE_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - CALIBRAE_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db.head_block_time() + ( CALIBRAE_OWNER_AUTH_RECOVERY_PERIOD - CALIBRAE_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      CALIBRAE_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      CALIBRAE_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      CALIBRAE_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db.head_block_time() + CALIBRAE_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( CALIBRAE_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      CALIBRAE_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      CALIBRAE_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      CALIBRAE_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      tx.set_expiration( db.head_block_time() + CALIBRAE_BLOCK_INTERVAL );
      tx.sign( bob_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
      ...
      CALIBRAE_REQUIRE_THROW( db.get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + CALIBRAE_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.nebula_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db.head_block_time() + CALIBRAE_BLOCK_INTERVAL;
      et_op.escrow_expiration = db.head_block_time() + 2 * CALIBRAE_BLOCK_INTERVAL;
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      CALIBRAE_REQUIRE_THROW( db.get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_savings_withdraw( "alice", op.request_id ).complete == db.head_block_time() + CALIBRAE_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      generate_blocks( db.head_block_time() + CALIBRAE_SAVINGS_WITHDRAW_TIME - fc::seconds( CALIBRAE_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < CALIBRAE_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = CALIBRAE_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db.get_account( "alice" ).savings_withdraw_requests == CALIBRAE_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db.head_block_time() + CALIBRAE_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = CALIBRAE_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( account_bandwidth )
{
   try
   {
      ...
      BOOST_REQUIRE( average_bandwidth == fc::raw::pack_size( tx ) * 10 * CALIBRAE_BANDWIDTH_PRECISION );
      ...
      BOOST_REQUIRE( last_bandwidth_update == db.head_block_time() );
      BOOST_REQUIRE( average_bandwidth == total_bandwidth + fc::raw::pack_size( tx ) * 10 * CALIBRAE_BANDWIDTH_PRECISION );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      CALIBRAE_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( account_create_with_delegation_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( delegation->min_delegation_time == db.head_block_time() + CALIBRAE_CREATE_ACCOUNT_DELEGATION_TIME );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / CALIBRAE_100_PERCENT );
      BOOST_REQUIRE( itr->rshares == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / CALIBRAE_100_PERCENT );
      ...
      tx.set_expiration( db.head_block_time() + CALIBRAE_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( sam_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( exp_obj->expiration == db.head_block_time() + CALIBRAE_CASHOUT_WINDOW_SECONDS );
      ...
      generate_blocks( exp_obj->expiration + CALIBRAE_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), CALIBRAE_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      CALIBRAE_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), CALIBRAE_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), CALIBRAE_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      CALIBRAE_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", CALIBRAE_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", CALIBRAE_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      CALIBRAE_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", CALIBRAE_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", CALIBRAE_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      CALIBRAE_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), CALIBRAE_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( CALIBRAE_INIT_MINER_NAME + fc::to_string( i ) ), CALIBRAE_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), CALIBRAE_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * CALIBRAE_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * CALIBRAE_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * CALIBRAE_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.sign( alice_private_key, db.get_chain_id() );
      CALIBRAE_REQUIRE_THROW( db.push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - CALIBRAE_BLOCK_INTERVAL );
