...
BOOST_AUTO_TEST_CASE( nebula_inflation )
{
   ...
   /*
      ...
      auto new_rewards = std::max( STEEMIT_MIN_CONTENT_REWARD, asset( ( STEEMIT_CONTENT_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), NECTAR_SYMBOL ) )
         + std::max( STEEMIT_MIN_CURATE_REWARD, asset( ( STEEMIT_CURATE_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), NECTAR_SYMBOL ) );
      auto witness_pay = std::max( STEEMIT_MIN_PRODUCER_REWARD, asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), NECTAR_SYMBOL ) );
      ...
      while( db.head_block_num() < STEEMIT_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( STEEMIT_MIN_CONTENT_REWARD, asset( ( STEEMIT_CONTENT_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), NECTAR_SYMBOL ) )
            + std::max( STEEMIT_MIN_CURATE_REWARD, asset( ( STEEMIT_CURATE_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), NECTAR_SYMBOL ) );
         witness_pay = std::max( STEEMIT_MIN_PRODUCER_REWARD, asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), NECTAR_SYMBOL ) );
      ...
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / STEEMIT_BLOCKS_PER_YEAR ) * STEEMIT_1_PERCENT ) / STEEMIT_100_PERCENT, NECTAR_SYMBOL ) * ( vesting_shares / vesting_steem ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      asset sbd_balance = asset( ( gpo.virtual_supply.amount * ( STEEMIT_SBD_STOP_PERCENT + 30 ) ) / STEEMIT_100_PERCENT, NECTAR_SYMBOL ) * exchange_rate;
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_account( STEEMIT_NULL_ACCOUNT ).reward_nebula_balance == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_account( STEEMIT_NULL_ACCOUNT ).reward_nebula_balance == ASSET( "0.000 TESTS" ) );
