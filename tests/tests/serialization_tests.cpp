...
BOOST_AUTO_TEST_CASE( version_test )
{
   try
   {
      ...
      CALIBRAE_REQUIRE_THROW( fc::from_variant( ver_str, ver ), fc::exception );
...
{
   signed_block b;
   while( b.witness.length() < CALIBRAE_MIN_ACCOUNT_NAME_LENGTH )
      b.witness += 'a';
   size_t min_size = fc::raw::pack_size( b );
   BOOST_CHECK( min_size == CALIBRAE_MIN_BLOCK_SIZE );
}
